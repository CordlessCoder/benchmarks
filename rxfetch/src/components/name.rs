...
use std::{ffi::CStr, fmt::Debug, mem::MaybeUninit, ptr};
...
impl<B> PwuId<B> {
    ...
    #[must_use]
    pub fn password(&self) -> DisplayBytes<'_> {
...
impl<Buf: GrowableBackingBuffer> PwuId<Buf> {
    ...
    #[cfg_attr(feature = "tracing", tracing::instrument(level = "debug", skip(buf), fields(buf_cap = buf.as_ptr_cap().1)))]
...
impl<Buf: BackingBuffer> PwuId<Buf> {
    ...
    pub fn try_get(mut buf: Buf, uid: u32) -> Result<Self, (PwuIdErr, Buf)> {
        ...
        let err =
            unsafe { libc::getpwuid_r(uid, &raw mut passwd, ptr.cast(), cap, &raw mut resultp) };
        use PwuIdErr::*;
        if resultp.is_null() {
            let err = match err {
...
fn up_to_null(slice: &[i8]) -> &[u8] {
    ...
    let slice: &[u8] = unsafe { &*(slice as *const [i8] as *const [u8]) };
...
impl SystemName {
    ...
    pub fn get() -> std::io::Result<Self> {
        let init = || unsafe { core::mem::zeroed::<libc::utsname>() };
        let mut uname = init();
