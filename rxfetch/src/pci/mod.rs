...
impl PciDevice<NoProvider> {
    const fn new(domain: u16, bus: u8, device: u16, function: u8) -> Self {
        PciDevice {
            domain,
            bus,
            device,
...
pub enum PciBackendError {
    #[error("PCI Device Not Available")]
    NotAvailable,
    #[error("IO Error: {0}")]
...
impl PciDevIterBackend for PciAutoIter {
    ...
    fn try_init() -> Result<Self, PciBackendError> {
        let sysfs = || SysBusBackend::try_init().map(PciAutoIter::SysFS);
